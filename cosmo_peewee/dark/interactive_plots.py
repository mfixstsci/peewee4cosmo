...
def plot_time(detector, dark, date, temp, solar, solar_date, outname):
    ...
    s1 = figure(width=plt_wth, height=plt_hgt, x_range=(min_date, max_date), 
                title='{} Global Dark Rate as of {} EST'\
                .format(detector, strftime("%m-%d-%Y %H:%M:%S", 
                                           localtime())), tools=TOOLS)
    
    s1.select(dict(type=HoverTool)).tooltips = {"Date":"$x", "Dark Rate":"$y"}
    s1.title.text_font_size = '15pt'
    s1.circle(date, dark, legend='Dark Count Rate',size=4, 
    ...
    if detector == 'NUV':
        ...
        # Plot Solar Flux
        s2 = figure(width=plt_wth, height=plt_hgt, x_range=s1.x_range, 
                    title=None, tools=TOOLS)
        s2.select(dict(type=HoverTool))\
                  .tooltips = {"Date":"$x", "Solar Flux":"$y"}
        s2.line(solar_date, solar, legend='10.7 cm', color='gold', 
                line_width=2)
        s2.line(solar_date[:-41], solar_smooth[:-41], legend='10.7 cm smoothed', 
        ...
        # Plot Temperture
        s3 = figure(width=plt_wth, plot_height=plt_hgt, x_range=s1.x_range, 
                    title=None, tools=TOOLS)
        s3.select(dict(type=HoverTool))\
                  .tooltips = {"Date":"$x", "Temperture":"$y"}
