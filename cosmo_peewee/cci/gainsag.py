...
from .gainmap_sagged_pixel_overplotter import make_overplot, \
                                              hotspot_plotter_interactive, \
                                              gsagtab_overplot_comparison
...
def main(out_dir, hotspot_filter=True):
    ...
    webpage_dir = settings['webpage_location']
    ...
    partial = functools.partial(make_all_gainmaps, 
                                gainmap_dir=os.path.join(monitor_dir,'CCI'))
    ...
    all_gainmaps = glob.glob(os.path.join(monitor_dir, 'CCI', 
                                          'total_gain_???.fits'))
    pool.map(make_webpage_plots, all_gainmaps)

    # Move to webpage
    webpage_figures = glob.glob(os.path.join(monitor_dir, 'CCI', 
                                             'cumulative_gainmap_FUV?_*.png'))
    for figure in webpage_figures:
        # Set up destination path
        dst_path = os.path.join(webpage_dir, 'cci', 
    ...
    make_all_gainmaps(100, gainmap_dir=os.path.join(monitor_dir,'CCI'), 
                      start_mjd=55055, end_mjd=70000, total=True)
    ...
    for new_gsagtab, calcos_tab in zip(gsagtabs, cdbs_gsagtabs):
        ...
        # Getting weird matplotlib 'cant load charcode error...'
...
def gsagtab_extension(date, lx, dx, ly, dy, dq, dethv, hv_string, segment):
    ...
    tab = fits.BinTableHDU.from_columns(
                            [date_col,lx_col,ly_col,dx_col,dy_col,dq_col])
...
def make_gsagtab_db(out_dir, blue=False, filter=True, by_date=False, tab_date=57322.0):
    ...
    descrip_string = 'Gives locations of gain-sag regions as of %s'\
    ...
    hdu_out[0].header.add_history('%d given current lower pulse height\
                                         filtering.'%(MODAL_GAIN_LIMIT))

    # Possible HVs for all of the extensions.
    possible_hv_strings = ['000', '100'] + list(map(str, list(range(142, 
                                                                    179))))
    ...
    for segment in segments:
        ...
        for hv_level in possible_hv_strings:
            # Set all of the empty lists that will be the data arrays
            # for GSAGTAB exts.
            date = []
            lx = []
            dx = []
            ...
            if filter and not by_date:
                # Get all of the sagged pixels for a specific 
                # Segment/HV Level combo.
                flagged_pix = list(Flagged_Pixels.select().\
                                        distinct().where(
            ...
            # for each x,y coodinate pair, create a dictionary where 
            # there key is the x,y pair that contains a list of dictionaries
            ...
            for coord_pair in result.keys():
                ...
                # Block for blue mode creation. in_boundary checks for LP1 
                # gsag holes that are in the LP2 extraction regions. We want 
                # to exclude them because we get low counts in the wings and 
                # since blue modes use boxcar extraction it will flag the whole 
                # column and we want to avoid that. According to the team, the 
                # long term solution would be to only have one gsagtab and LP2 
                # will use 2zone extraction.
                if blue and in_boundary(row_bad['segment'], 
                                        row_bad['y']*Y_BINNING, Y_BINNING):
                    logger.debug("EXCLUDING FOR BLUE MODES: {} {} {}".format(
                                                            row_bad['segment'], 
                                                            row_bad['y']*\
                                                            Y_BINNING, 
                                                            Y_BINNING))
            ...
            tab = gsagtab_extension(date, lx, dx, ly, dy, dq, hv_level, 
                                    hvlevel_string, segment)
            hdu_out.append(tab)
    ...
    if blue:
        ...
        descrip_string = 'Blue-mode gain-sag regions as of %s'\
                          %(str(datetime.now().date()))
        while len(descrip_string) < 67:
            descrip_string += '-'
        hdu_out[0].header['DESCRIP'] = descrip_string
...
def check_pixel_recovery(segment):
    ...
    flagged_pixels = Flagged_Pixels.select().\
                            distinct().where(
                                            (Flagged_Pixels.recovery == False) &
    ...
    gain_measurements = list(Gain.select().\
                                distinct().where(
    ...
    logger.info('FOUND {} SAGGED PIXELS FOR SEGMENT {}!'\
                .format(len(gain_measurements),segment))
    ...
    for combo in result.keys():
        ...
        if latest_measurement['gain'] > 3:
            Flagged_Pixels.update(recovery=True)\
                                .where(
            ...
            logger.info('UPDATING ROW FOR x={}, y={}! gain={} AS OF {}'\
                        .format(latest_measurement['x']*X_BINNING,
                                latest_measurement['y']*Y_BINNING,
                                latest_measurement['gain'],
                                Time(latest_measurement['expstart'],
                                     format='mjd').iso)) 
        # Else, the pixel never recovered keep the entry.
        else:
            logger.warning('SEGMENT{}, PIXEL x={}, y={} IS SAGGED! gain={} AS OF {}.'\
                           .format(latest_measurement['segment'],
                                   latest_measurement['x']*X_BINNING,
                                   latest_measurement['y']*Y_BINNING,
                                   latest_measurement['gain'],
                                   Time(latest_measurement['expstart'],
                                        format='mjd').iso))
...
def make_gsagtab_db_entry():
    ...
    parser.add_argument('--by_date',
                        type=bool,
                        default=False,
                        help="Do you want plan to make a table up to a certain \
                             date?")
